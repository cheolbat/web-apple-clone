(() => {
    ...
    let prevScrollheight = 0; // 현재 스크롤 위치(yOffset)보다 이전에 위치한 스크롤 섹션들의 스크롤 높이값의
    ...
    const sceneInfo = [
        {
            //  0
            type: "sticky",
            heightNum: 5, //브라우저 높이의 5배로 scrollHeight 세팅 (각각의 기기 마다 브라우저 높이가 다르기 때문에 브라우저 높이를 먼저 불러오고 거기에 배수 처리를 해주면 비슷한 UX)
            scrollHeight: 0,
            objs: {
    ...
    function playAnimation() {
        ...
        switch (currentScene) {
            ...
            case 2:
                ...
                // canvas를 2번 section 부터 미리 그려주기. 갑자기 띡! 나오는 거 방지.
                if (scrollRatio > 0.9) {
                    ...
                    // const recalculatedInnerWidth = window.innerWidth -> 스크롤바 width 포함 -> 오차 생김 / canvasScaleRatio;
            ...
            case 3:
                ...
                if (!values.rectStartY) {
                    ...
                    values.rect2X[2].start =
                        window.innerHeight / 2 / scrollHeight;
                    values.rect1X[2].end =
                        values.rectStartY / sceneInfo[3].scrollHeight;
                    values.rect2X[2].end =
                        values.rectStartY / sceneInfo[3].scrollHeight;
                }
                //좌표 구하기
...
// () => {} = function() {}
// 한번 감싸는 이유, 전역변수 사용 지양(다른 js파일에서 접근할 수 있음->충돌 가능성), 따라서 함수 안에서 지역변수 선언
